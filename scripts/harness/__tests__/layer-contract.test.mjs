...
import { importsOf } from '../layer-common.mjs'
...
describe('layer contract scans (ARCH-001)', () => {
  ...
  it('reports side-effect imports and keeps parsing the imports after them', () => {
    expect(
      importsOf(
      ...
      { specifier: 'pg', typeOnly: false },
    ])
    write(
      'packages/content/src/index.ts',
      "import 'pg'\nimport sharp from 'sharp'\nexport const a = sharp\n",
    )
    const bare = run('scan-layer-imports.mjs')
    expect(bare.code).toBe(1)
    expect(bare.out).toContain(
      'packages/content/src/index.ts -> pg — contract may not import sdk module',
    )
    expect(bare.out).toContain(
      'packages/content/src/index.ts -> sharp — contract may not import sdk module',
    )
    write('packages/content/src/index.ts', 'export const a = 1\n')
    // The PLUG-001 probe: a relative side-effect import from a package script.
    write(
      'packages/claude-plugin/scripts/tsx-stub.mjs',
      "import '../../content/src/index.ts'\n",
    )
    const relative = run('scan-layer-imports.mjs')
    expect(relative.code).toBe(1)
    expect(relative.out).toContain(
      'packages/claude-plugin/scripts/tsx-stub.mjs -> ../../content/src/index.ts — plugin may not import contract',
    )
    rmSync(path.join(sandbox, 'packages/claude-plugin'), { recursive: true })
    expect(run('scan-layer-imports.mjs').code).toBe(0)
  })
