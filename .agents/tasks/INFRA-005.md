...
complete in the authenticated web UI and agent CLI/API. Activate Second Example
...
- [ ] TC-04: Provision and verify a separate Second Example static Pages deployment.
...
- The second publication's domain resolves to the static host's nameservers,
  while no matching Pages project exists yet (checked on an operated instance;
...
- Existing Example News data and production DNS are protected. New publication
  data uses only `second-site`-qualified records and object keys.
