...
# INFRA-005: Complete multi-publication management and Second Example activation

## Problem

The production platform currently exposes only the Example News public Pages
...
publication, second.example.com, must operate as a separately managed site
without sharing content, canonical URLs, public artifacts, comments, audit
history, or authorization scope with Example News. The management solution must
...
- Cloudflare Pages: a separate `second-site-public` static project and its
...
- Existing Neon PostgreSQL state, existing S3-compatible store, and the
  publication worker: only the `second-site` site namespace.
...
- [x] Sibling scan 완료 — existing `default-public`, `default`, and
      site-qualified paths were checked; `second-site` and
      `second-site-public` do not collide.
- [x] 대안 최소 2개 검토 완료
- [x] 결정 근거 문서화 완료
...
2. Migrate the site catalog once to PostgreSQL and remove the environment-only
   ...
   and bootstrap `second-site` from its own identity into an empty state;
   confirm that no Example News data is selected or copied.
...
4. Materialize an `second-site` release and deploy it only to a new Pages
   project.
5. After explicit owner confirmation, attach the verified Second Example domain
...
- `.agents/spec-docs/active/INFRA-005-second-site-activation.md`
...
- [ ] TC-01: The UI and authenticated `GET /api/v2/sites` response both list
      `default` and `second-site`; the same non-default record has the selected
      Second Example HTTPS canonical origin and no environment-only catalog is read.
- [ ] TC-02: An authenticated site-scoped automation request for
      `second-site` bootstraps exactly one blank state with its own configured
...
- [ ] TC-03: The publication worker produces a verified candidate manifest
      whose `siteId` is `second-site`, with all snapshot and media logical paths
      under `sites/second-site/`.
- [ ] TC-04: Cloudflare Pages lists `second-site-public` as a project separate
      from `default-public`, and its production deployment serves the verified
      `second-site` release without runtime database or admin requests.
- [ ] TC-05: After a recorded owner confirmation immediately before DNS
      mutation, Second Example' selected canonical hostname returns HTTP 200 with
      matching canonical/OG metadata, responsive desktop/mobile styling, and the
      standard static cache policy; `news.example.com` remains unchanged.
...
| ----- | ---------------------------- | -------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TC-01 | integration + browser        | PostgreSQL site-registry fixture, authenticated `GET /api/v2/sites`, and admin UI test             | Preconditions: migrated existing site plus newly created Second Example; assert the UI and machine response derive the same record and the environment-only path is absent.                                                                   |
...
| TC-03 | integration                  | Publication worker manifest inspection and S3-compatible object listing                            | Preconditions: only the Second Example site state is selected; verify `siteId` and logical prefixes, never raw provider credentials.                                                                                                          |
...
| TC-05 | manual + HTTP smoke          | Owner-confirmed Cloudflare DNS action, curl, and browser mobile/desktop inspection                 | Preconditions: owner grants narrow DNS confirmation after target records are shown; confirm both new hostname behavior and the unchanged Example News origin.                                                                                 |
...
Problem identifies the current single-publication exposure and the reproduction condition of activating Second Example alongside Example News; it contains no TBD/TODO placeholder.
...
Problem still identifies the single-publication condition and the incorrect cross-publication sharing outcome when Second Example is activated; it contains no TBD/TODO placeholder.
