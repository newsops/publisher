...
CLI, so replacing `ADMIN_AUTOMATION_KEYS` to add a narrowly scoped content agent for one
publication would silently revoke every existing automation key. This
