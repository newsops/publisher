...
contract. As a result, an operator of either publication (site `default` or
`second-site`) cannot establish a useful first-level section hierarchy and
...
5. Bootstrap, idempotently and only when absent, a starter set of first-level
   categories per site (the current generic set lives in
   `packages/content/src/managed-seed.ts`; operators define their own). Existing
...
- [x] TC-05: Idempotent bootstrap gives `default` and
      `second-site` the named first-level categories without deleting or
