...
| TC-06 | gate      | `pnpm typecheck`, `pnpm test`, `pnpm harness:scan`, worker run on real snapshots | The two production snapshots (all `bodyHtml`-only) converted with zero failures and zero `<div></div>` output     |
...
`pnpm typecheck` (8 packages), `pnpm test` (201 harness tests including 7 new), and `pnpm harness:scan` (6/6) exited 0. Both snapshots of an operated two-site instance (all posts without `bodyMarkdown`) resolved through `publicationInputs` with 0 failures and 0 empty-div artifacts (verified on an operated instance; evidence kept privately by the operator).
...
Gates exited 0 as recorded under GATE-VERIFY; the real-snapshot worker run printed `total <n> empty div artifacts 0` (evidence kept privately by the operator).
