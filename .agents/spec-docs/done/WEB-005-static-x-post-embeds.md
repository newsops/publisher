...
Current production HTML fetched from `https://aitrendtimes.com/2026/09/openai-pauses-new-pro-20x-subscriptions` contains the OpenAI-led title and two `data-publisher-x-post` cards, while containing no `platform.twitter.com`, iframe, or Cloudflare beacon string. The earlier CSP beacon error is resolved by disabling the optional RUM beacon rather than weakening CSP.
...
Command: `pnpm exec vitest run scripts/harness/__tests__/x-oembed-contract.test.mjs` and authenticated production `publisher embed x resolve` calls for both Tibo canonical URLs.
