...
이 저장소는 프로그램만 담는다 (`.agents/rules/repository-scope.md`). 운영 중인
사이트에서의 확인이 필요한 TC는 **그 사이트를 식별하지 않고** 작성한다.

- Completion Criteria·Test Plan에는 플레이스홀더(`example.com`, 사이트 ID
  `default`/`example`)만 쓴다. 운영 사이트의 이름·도메인·호스트, 사이트 ID,
  배포/운영/릴리스 ID, 프로덕션 스크린샷 경로를 적지 않는다.
- 운영 인스턴스 확인이 필요한 TC의 Notes에는 "verified on an operated
  instance; evidence kept privately by the operator"로 기록될 것임을 적는다.
  실제 URL·ID·출력·스크린샷은 운영자의 비공개 노트에만 남는다.
