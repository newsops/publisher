...
애플리케이션 소유 계정·세션, 정적 공개 산출물의 단일 경로다.
...
## 비용 효율과 대규모 읽기 트래픽 목표

이 플랫폼의 운영 목표는 **무료 플랜 또는 Hobby 수준의 예산으로 시작하되,
읽기 트래픽은 엔터프라이즈급 규모까지 수평적으로 흡수하는 것**이다. 이는
저렴한 DB를 고르는 것만으로 달성하지 않는다. 공개 요청을 데이터베이스에서
분리하고, 검증된 정적 release를 일반 CDN으로 전달하는 구조가 비용과
확장성의 중심이다.

- 공개 기사·홈·카테고리·저자·월별 archive·feed·sitemap·검색 index·이미지는
  발행 때 정적 artifact로 materialize한다. 독자는 CDN cache에서 이를 읽으며,
  정상적인 공개 page-view가 PostgreSQL·관리자 runtime·private object store에
  연결을 만들면 안 된다.
- PostgreSQL은 관리자 로그인, 편집, 발행 상태, 감사, 댓글 제출·moderation처럼
  쓰기 또는 권한 검사가 필요한 경로에만 사용한다. 읽기 트래픽 증가가 DB 연결
  수·쿼리 비용·serverless 실행 횟수로 선형 전가되는 설계는 허용하지 않는다.
- 최신글·인기글·댓글처럼 변할 수 있는 정보도 우선 release에 SEO 가능한
  baseline을 포함한다. 갱신은 CDN 재검증 가능한 작은 pointer/projection 또는
  다음 정적 release로 처리하며, 실패해도 정적 본문과 핵심 탐색은 유지한다.
- 불변 해시 asset에는 장기 cache, HTML과 stable pointer에는 즉시 재검증 정책을
  적용한다. 이 HTTP 계약은 특정 CDN의 유료 기능이나 edge runtime에 의존하지
  않는다.
- Hobby/free allowance는 가용성·SLA·무제한 비용 상한의 동의어가 아니다. 사용량,
  백업, 복구를 관찰하고 한도를 넘는 순간에도 공개 읽기 경로는 DB scale-up이나
  새 유료 계층을 자동 도입하지 않는다. 새 비용 또는 계층은 명시적 운영 결정으로
  다룬다.

따라서 “엔터프라이즈급 트래픽”은 public read delivery의 목표이고, 무제한
쓰기·무제한 운영 지원을 약속하는 표현은 아니다. 쓰기 빈도가 늘어 PostgreSQL
또는 moderation 용량이 부족해지면 표준 PostgreSQL 교체·확장과 별도 비용
결정을 거치되, 이미 배포된 정적 사이트의 읽기 가용성은 그 변화와 독립적으로
유지한다.

## 사람·에이전트 동등 운영 방향 (에이전트 우선 운영 방향의 확장)

이 플랫폼의 장기 사용자는 브라우저를 조작하는 사람만이 아니다. 사람의 목표와
권한 아래에서 뉴스 사이트를 지속적으로 운영하는 에이전트도 일급 운영자다.
따라서 사람용 웹 UI와 에이전트용 CLI/API는 서로의 보조 수단이나 축소판이
아니다. 두 인터페이스는 같은 도메인 규칙·권한 경계·검증·operation 상태를
공유하되, 각각 독립적으로 매체 생성, 설정, 콘텐츠·미디어·분류 관리, 발행,
상태 확인, 복구와 rollback을 완결할 수 있는 일급 제어면이다.

- 웹 UI는 사람이 직접 운영할 때 모든 기능을 완결하는 인증된 운영면이다. CLI를
  설치하거나 API 요청을 수동 조합해야만 가능한 정상 운영 기능을 남기지 않는다.
- CLI와 인증된 API는 에이전트가 브라우저 화면을 해석하거나 클릭하지 않고 모든
  기능을 완결하는 버전된 기계 운영면이다. UI 전용 버튼, 불안정한 텍스트 파싱,
  숨은 browser-only 상태를 정상 경로로 두지 않는다.
- 새 매체의 등록·초기화·권한 부여·발행·공개 호스트 준비도 양쪽 제어면에서 같은
  idempotency key, operation ID, validation, audit 결과를 사용한다. 공급자 계정,
  비용, production DNS처럼 사람의 명시적 권한이 필요한 경계는 어느 제어면에서도
  `AUTHORITY_REQUIRED`와 필요한 다음 행동으로 표현한다.
- 한 제어면에서 기능을 추가하거나 변경하면, 다른 제어면의 동등 capability와
  성공·오류·권한 경계를 같은 변경에서 검증한다. 의도적으로 지원하지 않는 기능은
  두 제어면에서 명시적으로 비기능(non-goal)으로 기록하며, 조용한 기능 격차를
  허용하지 않는다.

에이전트 제어면은 다음 특성을 갖는 공개되고 버전된 기계 계약이다.
...
- admin 인증은 애플리케이션 소유 계정, 메모리-하드 암호 해시, 폐기 가능한
  opaque session을 계약으로 삼는다. 외부 IdP·Access는 필수 제공자가 아니며
  production fallback으로 사용하지 않는다.
