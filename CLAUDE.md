...
- Apply `.agents/rules/repository-scope.md`: this repository is the Publisher
  program. Publications operated with it (including the owner's) are separate
  works — never commit their names, domains, site IDs, hosts, deployment or
  operation IDs, production evidence, or the operator's local paths and
  personal details, and never let an operation's deployments write GitHub
  Deployments, Environments, or checks for this repository.
